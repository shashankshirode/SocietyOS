...
import { formatFacilityCurrency, formatFacilityOperatingHours, } from '../services/facilityDateTimeFormatter';
import { backgroundBorderStyle, backgroundColorStyle, facilityBookingStyles as styles, } from '../styles/facilityBooking.styles';
...
export function FacilityDetailScreen({ navigation, route }: Props) {
    ...
    if (resource.isLoading || !facility && !resource.error) {
        return (<FacilityScreenLayout title={msg.spaceFocusTitle} onBack={navigation.goBack}>
        <Skeleton height={200}/>
        <DetailBlockSkeleton />
        <DetailBlockSkeleton />
    ...
    if (resource.error || !facility) {
        return (<FacilityScreenLayout title={msg.spaceFocusTitle} onBack={navigation.goBack}>
        <ErrorState title={labels.states.loadDetailsTitle} message={labels.states.loadDetailsDescription} onRetry={() => void resource.refresh()} retryLabel={labels.states.retry}/>
      </FacilityScreenLayout>);
    }
    ...
    const footer = isAccessBlocked || isMaintenance ? (<StickyFooter>
      <View style={styles.stickyButtonRow}>
        <AppButton title={msg.exploreOtherSpaces} onPress={() => navigation.navigate('FacilityList')} style={styles.stickyButton} variant="primary"/>
      </View>
    </StickyFooter>) : (<StickyFooter>
      <View style={styles.stickyButtonRow}>
        <AppButton title={msg.seeFullSchedule} onPress={() => navigation.navigate('FacilitySlotAvailability', { facilityId: facility.id })} variant="outline" style={styles.stickyButton}/>
        <AppButton title={!eligible ? msg.notEligible : selectedSlotHour ? msg.reserveSlotHour(selectedSlotHour) : msg.reserveAnHour} onPress={() => navigation.navigate('FacilitySlotAvailability', { facilityId: facility.id })} style={styles.stickyButton} disabled={!eligible || !selectedSlotHour}/>
      </View>
    </StickyFooter>);
    return (<FacilityScreenLayout title={msg.spaceFocusTitle} subtitle={facility.name} onBack={() => performBackNavigation(navigation, { fallbackRoute: 'FacilityList', currentRouteName: 'FacilityDetail' })} footer={footer} testID="facility-detail-screen">
      ...
      {!isMaintenance && !isAccessBlocked ? (<View style={[
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.timeRibbonTrack}>
            {['7:00 AM', '8:00 AM', '11:00 AM', '4:00 PM', '6:00 PM', '7:00 PM', '8:00 PM'].map((slot) => {
                ...
                return (<Pressable key={slot} onPress={() => setSelectedSlotHour(isSelected ? null : slot)} accessibilityRole="button" accessibilityLabel={`Select hour ${slot}`} accessibilityState={{ selected: isSelected }} testID={`slot-${slot}`} style={[
                        styles.timeSlotNode,
                        isSelected ? styles.timeSlotNodeSelected : null,
                        backgroundBorderStyle(isSelected ? colors.primarySoft : colors.background, isSelected ? colors.primary : colors.border),
                    ]}>
      ...
      {facility.amenities.length > 0 ? (<View style={{ gap: 8 }}>
          <SafeText variant="tiny" color="secondary" style={{ fontWeight: '700', letterSpacing: 0.5 }}>
            {msg.includedAmenities}
          </SafeText>
          <View style={styles.amenityGrid}>
            {facility.amenities.map((amenity) => (<View key={amenity.id} style={[styles.amenity, backgroundBorderStyle(colors.surface, colors.border)]}>
      ...
      <AppCard variant="outlined" style={styles.policyCard}>
        <View style={styles.rowBetween}>
          ...
          {facility.rules.length > 3 ? (<Pressable onPress={() => setShowAllRules(!showAllRules)}>
        ...
        {visibleRules.map((rule) => (<View key={rule.id} style={styles.policyRow}>
