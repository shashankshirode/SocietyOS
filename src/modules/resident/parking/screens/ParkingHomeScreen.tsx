...
import { styles, createBgStyle, createColorStyle, createContentInset, } from '../styles/MobilityField.styles';
export function ParkingHomeScreen({ navigation, route }: ParkingHomeScreenProps) {
    ...
    const heroNarrative = useMemo(() => {
        if (isBlocked)
            return 'Your exit path may be blocked.';
        if (isWrongVehicle && policy === 'FIXED_ALLOTMENT')
            return 'Another vehicle may be in your space.';
        ...
        if (policy === 'OPEN_COMMON_POOL') {
            if (!primaryVehicle)
                return 'Common resident parking area.';
            if (isAway)
                return `${primaryVehicle.makeModel || 'Your vehicle'} is away from society.`;
            return `${primaryVehicle.makeModel || 'Your vehicle'} is inside common parking.`;
        }
        if (policy === 'MECHANICAL_STACK') {
            if (!primaryVehicle)
                return 'Mechanical stack bay is ready.';
            if (isAway)
                return `Stack bay is empty. ${primaryVehicle.makeModel} is away.`;
            return `Your ${primaryVehicle.makeModel} is in mechanical stack.`;
        }
        if (!primaryVehicle)
            return 'Your parking space is waiting.';
        if (isAway)
            return `Your parking space is waiting. ${primaryVehicle.makeModel || 'Your car'} is away.`;
    ...
    const handleOpenFocus = (vehicle?: Vehicle) => {
        setSelectedVehicle(vehicle || primaryVehicle || null);
    ...
    const handleIncidentSubmit = async (incidentData: {
        type: 'WRONG_PARKING' | 'BLOCKED_VEHICLE';
        targetPlate: string;
        notes: string;
    }) => {
    ...
    return (<SocietyExperienceFrame>
      <View style={[styles.root, createBgStyle(theme.semantic.surface.canvas)]}>
        <ScrollView contentContainerStyle={[styles.content, createContentInset(0, bottomObstruction)]} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} tintColor={theme.semantic.accent.moss}/>}>
          ...
          <MobilityField policy={policy} primaryVehicle={primaryVehicle} assignedSlot={assignedSlot} activeIncident={activeIncident} onVehiclePress={(v) => handleOpenFocus(v)} onSlotPress={() => handleOpenFocus(primaryVehicle)} onReportIssuePress={() => handleReportIncident('WRONG_PARKING')} onRequestAllotmentPress={() => navigation.navigate('VisitorParkingRequest', { unitId })}/>
          ...
          <View style={{ gap: 8 }}>
            ...
            {isBlocked || isWrongVehicle ? (<PressableScale onPress={() => navigation.navigate('ParkingIncidentList', { unitId })} accessibilityRole="button" accessibilityLabel="Track parking issue">
                <View style={[
                styles.actionCard,
              ...
              </PressableScale>) : (<PressableScale onPress={() => navigation.navigate('VisitorParkingRequest', { unitId })} accessibilityRole="button" accessibilityLabel="Add visitor parking">
                <View style={[
                styles.actionCard,
          ...
          <MobilityTrace events={traceEvents} onEventPress={(e) => {
            if (e.type === 'INCIDENT') {
        ...
        <VehicleFocusSheet visible={focusSheetVisible} vehicle={selectedVehicle || primaryVehicle} assignedSlot={assignedSlot} allVehicles={data?.vehicles} onClose={() => setFocusSheetVisible(false)} onSelectVehicle={(v) => setSelectedVehicle(v)} onReportBlocked={() => handleReportIncident('BLOCKED_VEHICLE')} onReportWrongParking={() => handleReportIncident('WRONG_PARKING')}/>

        
        <MobilityIncidentModal visible={incidentModalVisible} mode={incidentMode} slotNumber={slotLabel} onClose={() => setIncidentModalVisible(false)} onSubmit={handleIncidentSubmit}/>
      </View>
