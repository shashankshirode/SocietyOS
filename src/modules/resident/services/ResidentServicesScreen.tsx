...
import {
  ...
  createTextStyle,
  styles,
} from './styles/ResidentServicesScreen.styles';
...
export function ResidentServicesScreen({ navigation }: Props) {
  ...
  const commands = useMemo<Command[]>(() => {
    ...
    // Filter commands by active capability availability
    const filtered = allCommands.filter((command) => canAccessCapability(command.capability));
    // Fallback: if all capability checks fail in preview, show the core safe set
  ...
  return (
    <SocietyExperienceFrame>
      <View style={[styles.root, createRootStyle(theme.semantic.surface.canvas)]}>
        <ScrollView
          contentContainerStyle={[styles.content, createContentStyle(0, bottomObstruction)]}
            ...
            {commands.map((command, index) => (
              <View key={command.id}>
                <Pressable
                  ...
                  accessibilityLabel={`${command.title}. ${command.description}`}
                  style={[
                    styles.commandRow,
                ...
                >
                  <View style={[styles.commandIcon, createSurfaceStyle(command.primary ? theme.semantic.accent.moss : theme.semantic.surface.soft)]}>
                    <Ionicons
                      name={command.icon}
                      size={22}
                      color={command.primary ? theme.semantic.text.inverse : theme.semantic.accent.moss}
                    />
                  </View>
                ...
                {!command.primary && index < commands.length - 1 ? (
                  <View style={[styles.divider, createSurfaceStyle(theme.semantic.border.subtle)]} />
                ) : null}
              </View>
            ))}
          </View>
