...
describe('FacilityDetailScreen Bottom Clearance & CTA States', () => {
  ...
  const route = { params: { facilityId: 'fac-detail-1' } } as any;
  ...
  it('disables "Reserve an Hour →" button until an hour slot is selected', async () => {
    await renderWithProviders(<FacilityDetailScreen navigation={navigation} route={route} />);

    // Screen title and facility info
    expect(screen.getAllByText('Clubhouse Badminton Court').length).toBeGreaterThan(0);

    // CTA button is present and disabled
    ...
    // Select an hour from the horizon track
    const slot4PM = screen.getByTestId('slot-4:00 PM');
    await act(async () => {
      fireEvent.press(slot4PM);
    });

    // CTA updates and becomes enabled
    ...
    // Pressing active button navigates to slot availability
    await act(async () => {
