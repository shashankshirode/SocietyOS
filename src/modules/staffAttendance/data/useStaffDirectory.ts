...
export function useStaffDirectory(filters?: Record<string, string>) {
  ...
  const fetch = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await staffAttendanceRepository.getStaffDirectory(filtersHandle.valueRef.current);
      setData(result);
    } catch (e) {
