...
export interface EmergencyIncident {
    ...
    readonly updatedAtIso: string;
    readonly locationNotes?: string | Absent;
}
...
class EmergencyRoutingService {
    ...
    public async updateHouseholdEmergencyPolicy(unitId: string, contacts: readonly HouseholdEmergencyContact[]): Promise<HouseholdEmergencyPolicy> {
    ...
    public async triggerEmergency(actorContext: ActiveContext, category: EmergencyCategory = 'GENERIC_SOS', locationNotes?: string): Promise<EmergencyIncident> {
        ...
        await domainEventBus.emit({
            ...
            payload: {
                ...
                routingSnapshot: {
                    societyGroups: [...incident.routingSnapshot.societyGroups],
                    resolvedRecipients: [...incident.routingSnapshot.resolvedRecipients],
                },
            },
        });
        return incident;
    }
    public async acknowledgeEmergency(responderContext: ActiveContext, incidentId: string, notes?: string): Promise<EmergencyIncident | null> {
        ...
        const effectiveLocationNotes = notes ?? existing.locationNotes;
        const updated: EmergencyIncident = {
            ...
            updatedAtIso: new Date().toISOString(),
            ...(effectiveLocationNotes ? { locationNotes: effectiveLocationNotes } : {}),
        };
    ...
    public getIncidentById(id: string): EmergencyIncident | null {
        return this.activeIncidents.get(id) ?? null;
    }
    public clear(): void {
