...
export type EmergencyState = 'CHOOSING' | 'RESPONSE_SELECTED' | 'SENDING' | 'ACTIVE' | 'ACKNOWLEDGED' | 'FAILED' | 'CANCELLED';
...
export function EmergencyExperience({ visible, onClose }: EmergencyExperienceProps) {
    ...
    useEffect(() => {
        ...
        if (!visible || !eventId || (state !== 'ACTIVE' && state !== 'ACKNOWLEDGED'))
            return;
    ...
    return (<View style={styles.root} accessibilityViewIsModal>
      ...
      <SafeAreaView style={styles.safe} edges={['top', 'bottom', 'left', 'right']}>
        ...
        <View style={styles.topBar}>
          ...
          {state !== 'SENDING' && (<Pressable onPress={onClose} accessibilityRole="button" accessibilityLabel={state === 'ACTIVE' || state === 'ACKNOWLEDGED' ? copy.returnToApp : copy.cancel} style={styles.closeButton}>
        ...
        {isTablet ? (<View style={styles.tabletWorkspace}>
            <View style={styles.tabletLeftRegion}>
              <EmergencyLens state={state} selectedType={selectedType} definitions={definitions} unitName={displayUnit} activeEvent={event} onSelectType={handleSelectType} onSelectGenericSos={handleSelectGenericSos}/>
            </View>
            ...
            <View style={styles.tabletRightRegion}>
              ...
              {state === 'RESPONSE_SELECTED' && recipientResolution.resolution?.resolvedRecipients ? (<ResponsePath unitName={displayUnit} recipients={recipientResolution.resolution.resolvedRecipients} accentColor={selectedDef.accentColor}/>) : null}

              <EmergencyCommand state={state} requestLabel={selectedDef.requestLabel} accentColor={selectedDef.accentColor} onRequest={() => void handleSendRequest()} onChangeResponse={() => setState('CHOOSING')} onCancelRequest={() => void handleCancelRequest()} onReturnToHome={onClose} onRetry={() => void handleSendRequest()} onCallGateSecurity={() => { }}/>
            </View>
          </View>) : (<ScrollView contentContainerStyle={styles.phoneScrollContent} showsVerticalScrollIndicator={false} bounces={false}>
            ...
            {state === 'RESPONSE_SELECTED' && recipientResolution.resolution?.resolvedRecipients && (<View style={styles.responsePathSection}>
                <ResponsePath unitName={displayUnit} recipients={recipientResolution.resolution.resolvedRecipients} accentColor={selectedDef.accentColor}/>
              </View>)}
