    ...
    await renderWithProviders(
      <ResidentHomeContextProvider>
        <SocietyExperienceProvider>
          <CreateVisitorPassScreen navigation={mockNavigation} route={mockRoute} />
        </SocietyExperienceProvider>
      </ResidentHomeContextProvider>
    );

    // 1. Centralized header container is rendered
    expect(screen.getByTestId('resident-header-container')).toBeTruthy();

    // 2. Residence Beacon displays society and unit context
    expect(screen.getByText(/Green Valley Heights/i)).toBeTruthy();

    // 3. Identity Orb is rendered
    expect(screen.getByTestId('resident-identity-orb')).toBeTruthy();

    // 4. Narrative heading is rendered in the page body
