...
export function SpaceHorizon({ facilities, onSelectFacility }: SpaceHorizonProps) {
  ...
  // Derive horizon nodes from facilities considering active resident role and admin policy
  const horizonItems: SpaceHorizonItem[] = facilities.slice(0, 6).map((f) => {
    const isRoleRestricted = !f.eligibilityPolicy.allowedRoles.includes(activeContext.residentRole as any);
    const isUnitBlocked = f.eligibilityPolicy.blockedUnitIds?.includes(activeContext.unitId) || false;
    const isTowerBlocked = f.eligibilityPolicy.blockedTowersOrWings?.some((t) =>
  ...
  return (
    <View
      ...
      accessibilityLabel={msg.availabilityHorizonTitle}
    >
      ...
      <ScrollView
        ...
        contentContainerStyle={styles.horizonTrack}
      >
        {horizonItems.map((item) => {
          ...
          return (
            <Pressable
              ...
              accessibilityLabel={`${item.name}, ${item.statusLabel}, ${item.timeHighlight}`}
              style={({ pressed }) => [
                styles.horizonNode,
                backgroundBorderStyle(
                  pressed ? colors.primarySoft : colors.background,
                  colors.border
                ),
              ]}
            >
