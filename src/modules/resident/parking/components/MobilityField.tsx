...
export type MobilityFieldProps = {
  policy?: SocietyParkingAllocationPolicy;
  primaryVehicle?: Vehicle | undefined;
  assignedSlot?: ParkingSlot | undefined;
  activeVisitorPass?: VisitorParkingPass | undefined;
  activeIncident?: ParkingIncident | undefined;
  onVehiclePress?: (vehicle: Vehicle) => void;
  onSlotPress?: (slot: ParkingSlot) => void;
  onVisitorSlotPress?: (pass: VisitorParkingPass) => void;
  onReportIssuePress?: () => void;
  onRequestAllotmentPress?: () => void;
};
...
export function MobilityField({
  ...
  onVisitorSlotPress,
  onReportIssuePress,
  onRequestAllotmentPress,
}: MobilityFieldProps) {
  const theme = useAppTheme();

  // Determine current mobility state
  ...
  return (
    ...
    >
      ...
      {/* 3. Spatial Bays Row (Home Bay + Optional Visitor Bay) */}
      <View style={styles.spatialBaysContainer}>
        {/* Scenario A: No Parking Society / No Allotment */}
        {isNoParkingSociety || (hasNoSlot && hasNoVehicle) ? (
          <Pressable
            onPress={onRequestAllotmentPress}
            accessibilityRole="button"
            accessibilityLabel="No parking space allotted. Request parking allotment."
            style={({ pressed }) => [
              styles.baySlot,
              styles.baySlotEmpty,
              createBgStyle(
                pressed ? theme.semantic.surface.soft : theme.semantic.surface.raised,
          ...
          /* Primary Assigned Bay / Common Pool Bay */
          <Pressable
            onPress={() => {
            ...
            accessibilityRole="button"
            accessibilityLabel={`Parking space ${slotLabel}. ${isParked ? `${vehicleName} parked.` : 'Empty.'}`}
            style={({ pressed }) => [
              styles.baySlot,
              ...
              createBgStyle(
                ...
                isBlocked || isWrongVehicle ? theme.semantic.status.warning : theme.semantic.accent.moss
              ),
            ]}
          >
            {/* Bay Header */}
            ...
            {isParked ? (
              <View style={styles.vehicleGlyphContainer}>
                <Ionicons
                  name={primaryVehicle?.vehicleType === 'TWO_WHEELER' ? 'bicycle' : 'car'}
                  size={34}
        ...
        {/* Active Visitor Bay (Rendered when visitor pass active) */}
        {activeVisitorPass ? (
          <Pressable
            onPress={() => onVisitorSlotPress?.(activeVisitorPass)}
            accessibilityRole="button"
            accessibilityLabel={`Visitor parking space ${activeVisitorPass.approvedParkingZone} for ${activeVisitorPass.visitorName}`}
            style={({ pressed }) => [
              styles.baySlot,
