...
export interface CommandOptions<TInput, TOutput> {
  ...
  onSuccess?: ((data: TOutput) => void) | undefined;
  onError?: ((error: DomainError | Error) => void) | undefined;
}
...
export interface CommandState<TInput, TOutput> {
  ...
  error: string | null;
  data: TOutput | null;
  execute: (input: TInput) => Promise<{ ok: boolean; data?: TOutput | undefined; error?: string | undefined }>;
  reset: () => void;
}
