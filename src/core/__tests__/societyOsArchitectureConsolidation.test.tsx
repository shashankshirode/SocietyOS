...
describe('Society OS Consolidated Architecture & Shared Primitives', () => {
    describe('1. SocietyButton & AsyncActionButton', () => {
        it('renders and triggers action on press', async () => {
            ...
            const screen = await renderWithProviders(<SocietyButton title="Save Entry" onPress={onPress} variant="primary" testID="society-btn"/>);
        ...
        it('AsyncActionButton prevents double tap during asynchronous execution', async () => {
            ...
            const screen = await renderWithProviders(<AsyncActionButton title="Submit Pass" onAction={onAction} testID="async-btn"/>);
    ...
    describe('2. SelectionGroup & Primitives', () => {
        it('renders options and selects key on tap', async () => {
            ...
            const screen = await renderWithProviders(<SelectionGroup label="Category" options={[
                    { key: 'GUEST', label: 'Guest Pass' },
                    { key: 'DELIVERY', label: 'Delivery' },
                ]} selected="GUEST" onSelect={onSelect}/>);
    ...
    describe('3. StatusIndicator', () => {
        it('renders with correct semantic status label', async () => {
            const screen = await renderWithProviders(<StatusIndicator label="Active" tone="active" testID="status-ind"/>);
    ...
    describe('4. ActionRow', () => {
        it('renders title, subtitle, and fires onPress', async () => {
            const onPress = jest.fn();
            const screen = await renderWithProviders(<ActionRow title="Notification Settings" subtitle="Configure alerts and SOS" onPress={onPress} testID="action-row"/>);
    ...
    describe('5. EmptyState & ErrorState', () => {
        it('renders empty state with call to action', async () => {
            ...
            const screen = await renderWithProviders(<EmptyState title="No passes issued" description="Create your first visitor access pass" actionTitle="Create Pass" onAction={onAction} testID="empty-state"/>);
        ...
        it('renders error state with retry action', async () => {
            ...
            const screen = await renderWithProviders(<ErrorState title="Network Connection Failed" message="Unable to reach society server." onRetry={onRetry} testID="error-state"/>);
    ...
    describe('6. ResidenceContextGuard', () => {
        it('blocks interactions and renders recovery warning when stale', async () => {
            ...
            const screen = await renderWithProviders(<ResidenceContextGuard isStale={true} onRestart={onRestart} message="Active flat changed. Please restart booking.">
        ...
        it('renders children directly when context is not stale', async () => {
            const screen = await renderWithProviders(<ResidenceContextGuard isStale={false}>
    ...
    describe('7. AdaptiveFlowComposer & SocietyPageShell', () => {
        it('renders flow title, content and sticky command in AdaptiveFlowComposer', async () => {
            const screen = await renderWithProviders(<AdaptiveFlowComposer title="Open your home" subtitle="Who are you opening your home to?" command={<SocietyButton title="Create Access" onPress={() => undefined}/>}>
        ...
        it('renders SocietyPageShell with bounded content and sticky action', async () => {
            const screen = await renderWithProviders(<SocietyPageShell showHeader={false} stickyAction={<SocietyButton title="Save Changes" onPress={() => undefined}/>} testID="test-page-shell">
    ...
    describe('8. Structured Error Presenter', () => {
        it('translates domain error codes into human messages', () => {
            expect(formatDomainError({ code: 'PERMISSION_DENIED', message: '' })).toBe('You do not have permission to perform this action.');
            expect(formatDomainError({ code: 'SLOT_UNAVAILABLE', message: '' })).toBe('The requested slot or resource is no longer available. Please select another.');
            expect(formatDomainError({ code: 'PASS_CANCELLED', message: '' })).toBe('This pass has been cancelled and cannot be used.');
            expect(formatDomainError({ code: 'STALE_STATE', message: '' })).toBe('Your active context has changed. Please refresh and try again.');
        });
    });
});
