...
export class ReportsMockSource {
  private mockFinancialReports = [...mockFinancialReports];

  async getReports(): Promise<RepositoryResult<FinancialReport[]>> {
    await withMockDelay();
    return repositorySuccess([...this.mockFinancialReports]);
  }
  ...
  async generateReport(name: string, month: string): Promise<RepositoryResult<FinancialReport>> {
    ...
    this.mockFinancialReports = [...this.mockFinancialReports, report];
  ...
  async getFinancialReport(id: string): Promise<RepositoryResult<FinancialReport | null>> {
    await withMockDelay();
    const report = this.mockFinancialReports.find(r => r.id === id) ?? null;
    return repositorySuccess(report);
  }
}